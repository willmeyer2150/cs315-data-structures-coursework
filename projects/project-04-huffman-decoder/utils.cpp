...
void exitOnError(error_type error, const std::string &entityName = "") {
    switch (error) {
        ...
        case HDR_PARSE_ERROR:
            std::cerr << "Error: HDR parsing failed. Terminating...\n";
            exit(HDR_PARSE_ERROR);

        case HDR_EMPTY:
            std::cerr << "Error: HDR empty. Terminating...\n";
            exit(HDR_EMPTY);

        case HDR_BAD_CODE_BITS:
            std::cerr << "Error: Bad code bits in " << entityName << ". Terminating...\n";
            exit(HDR_BAD_CODE_BITS);
...
std::string baseNameWithoutExt(const std::string& filename) {
    ...
    if (p.extension() == ".txt" || p.extension() == ".hdr" || p.extension() == ".code") {
