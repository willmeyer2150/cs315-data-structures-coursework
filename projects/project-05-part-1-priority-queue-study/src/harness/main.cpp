...
int main() {
    ...
    */
    ...
    // output csv file: batch_then_drain.csv and store in csvs directory
    std::ofstream out(csvPath);
    if (!out.is_open())
    {
        std::cerr << "Couldn't open " << csvPath << " for writing. Does ../csvs exist?\n";
        return 1;
    }

    out << RunResult::csv_header() << '\n';
    for (auto run: runResults) {
        out << run.to_csv_row() << '\n';
    }
    out.close();

    std::cout << "Wrote csv to " << csvPath << "\n";
